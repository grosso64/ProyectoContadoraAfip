...
{
    public partial class VerVentasAdmin : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string BuscarFactura = tBuscarFactura.Text.Trim();
            BindGridView(BuscarApellido, BuscarFactura);
        }

        protected void BindGridView(string apellido = "", string factura = "")
        {
            string idUsuario = Session["userId"].ToString();
            string cs = ConfigurationManager.ConnectionStrings["sincotDB"].ConnectionString;
            SqlConnection con = new SqlConnection(cs);
            ...
            string query = @"SELECT V.Id_Venta, V.Factura, V.Aclaracion, V.monto, V.fecha_venta, V.apellido_del_usuario, V.dni_del_usuario, A.FileName, A.FileData
                     ...
                     WHERE (@apellido = '' OR V.apellido_del_usuario LIKE @apellido)
                     AND (@factura = '' OR V.Factura LIKE @factura)";
        ...
        protected void gVer_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "DeleteVenta")
            {
                int idVenta = Convert.ToInt32(e.CommandArgument);
                DeleteVenta(idVenta);
                BindGridView(); // Actualiza el GridView después de la eliminación
            }
        }

        protected void DeleteVenta(int idVenta)
        {
            string cs = ConfigurationManager.ConnectionStrings["sincotDB"].ConnectionString;
            SqlConnection con = new SqlConnection(cs);

            string queryAdjuntos = "DELETE FROM Adjuntos WHERE Id_Venta = @idVenta";
            string queryVentas = "DELETE FROM Ventas WHERE Id_Venta = @idVenta";

            SqlCommand cmdAdjuntos = new SqlCommand(queryAdjuntos, con);
            SqlCommand cmdVentas = new SqlCommand(queryVentas, con);

            cmdAdjuntos.Parameters.AddWithValue("@idVenta", idVenta);
            cmdVentas.Parameters.AddWithValue("@idVenta", idVenta);
            ...
            try
            {
                ...
                cmdAdjuntos.Transaction = transaction;
                cmdVentas.Transaction = transaction;

                cmdAdjuntos.ExecuteNonQuery();
            ...
            {
                // Manejo de errores
                Label1.Text = "Error al eliminar la venta: " + ex.Message;
            }
            finally
            {
                con.Close();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!decimal.TryParse(montoText, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out monto))
            {
                ...
                {
                    lInfo.Text = "Monto inválido: " + montoText;
                    return;
                }
            }
            ...
            END";
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        BindGridView();
                        lInfo.Text = "Venta actualizada correctamente.";

                        // Obtener el ID del usuario de la venta
                        ...
                        if (idUsuario > 0)
                        {
                            ...
                            string mensaje = $"La venta con ID {idVenta} ha sido actualizada por el administrador con los siguientes valores:\n" +
                                             ...
                                             $"Monto: {monto}\n";
                            Utilidades.RegistrarLog(idUsuario, "Actualizar Venta Area Admin", $"Venta actualizada para el usuario {apellido}, con el dni: {dni}, numero de factura: {factura} ");
                            Utilidades.EnviarMailConAdjunto(email, "Actualización de Venta", mensaje, fileData, fileName);
                            Label2.Text = "Venta Actualizada";
                        }
                        else
                        {
                            Debug.WriteLine($"El valor : {idUsuario}");
                            Label2.Text = "No se pudo obtener el ID del usuario de la venta.";
                        
                        }
                    }
                    else
                    {
                        Label2.Text = "Error al actualizar la venta.";
                    }
                }
                catch (Exception ex)
                {
                    Label2.Text = "Error al actualizar la venta: " + ex.Message;
                }
            }
        }
        ...
        private int ObtenerIdUsuarioDeVenta(string idVenta, SqlConnection con)
        {
            ...
            Debug.WriteLine($"Valor de idVenta: {idVenta}");
         
            Debug.WriteLine($"Valor final de idUsuario: {idUsuario}");
